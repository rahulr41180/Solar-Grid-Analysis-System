...
function rayHitsBuilding(origin: THREE.Vector3, dir: THREE.Vector3, b: Building): boolean {
...
function analysePanel(
  ...
  const loss =
    shadedFraction + CONCENTRATION_ALPHA * stringPenalty + EOF_BETA * eof * shadedFraction;
...
export function analyseScene(
  ...
  const tableResults: TableAnalysis[] = tables.map((table) => {
    const panelResults: PanelAnalysis[] = layouts.map((layout) => {
      const { samples, normal } = panelSamplePoints(table, layout, SAMPLE_ROWS, SAMPLE_COLS);
      const grid: boolean[][] = [];
  ...
  return { tables: tableResults, sunDir, sunUp, computedAtMinutes };
}
